...
class AuthFlowNotifier extends Notifier<AuthFlowState> {
  ...
  void signIn(String email, String password) {
    if (email.trim().isEmpty || email.contains('missing')) {
      state = state.copyWith(signInError: 'No account found with this email');
    } else if (password != 'skiflux') {
      state = state.copyWith(signInError: 'Incorrect password');
    } else {
      state = state.copyWith(stage: AuthStage.fingerprint, clearError: true);
    }
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return switch (state.stage) {
      ...
      AuthStage.fingerprint || AuthStage.faceId => _BiometricScreen(
        ...
        onVerify: () => Navigator.of(context).pushReplacement(
